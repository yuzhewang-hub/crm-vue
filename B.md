# 项目名称：Awesome No
## 项目名称：Awesome Notes App2

## 📝 项目简介1

**Awesome Notes App1** 是一个简洁高效的笔记管理应用，不支持多端同步，助你轻松记录灵感、待办事项和工作笔记。

**Awesome Notes App1**


行内代码 `console.log('Hello, World!')` 渲染效果

行内代码 `console.log('Hello')` 渲染效果

> 注意：本项目仅供学习和参考，请勿用于商业用途。

> 注意：本项目仅供学习和参考，请勿用于商业用途。

## 📸 截图预览

[baidu](https://www.baidu.com)

[baidu](https://www.baidu.com)

---

![背景2](/background.png)

![图片](https://img2.baidu.com/it/u=3280885062,2707279109&fm=253&fmt=auto&app=138&f=JPEG?w=500&h=500)

![他仓图片](https://test.gitcode.net/litestabc/linzi-gaiming/raw/main/devadvocate%20(1).png)

![本仓相对图片](../docs/测试.jpg)

![本仓绝对图片](/docs/1.jpg)

- [ ] 未完成任务
- [ ] 未完成

## 🚀 功能特点

- ✍️ 支持 Markdown 格123式编辑
- 🌙 黑暗模式切换1
- 🔍 笔记全文搜索1
- 🔍 笔记全文搜索1
- 🔍 笔记全文搜索1

1. 第三步
2. 第二步
   1. 子步骤2
   2. 子步骤3

---

- 项目二212
  - 子项
- 项目二212
  - 子12项

| 姓名 | 年龄 | 城市 |
|------|------|------|
| 小布 |  22  | 北京 |
| 小红 |  20  | 上海 |
| 小红 |  20  | 上海 |
| 小红 |  23  | 上4 |
| 小红 |  23  | 上3 |
| 小红 |  24  | 上aaa |
| 小红 |  20  | 上b |
| 小红 |  25  | 上df |
| 小红 |  21  | 上海 |

> 注意：本项目仅供学习和参考，请用于商业123用途。

_1243_

_12123_


```javascript
console.log('Hello');
*123&&&1231
```

```css
body {
  background-color: #f0f0f0;
  background-color: #f0f0f0;
  https://test.gitcode.net/li2/recommend/issues?categorysearch=%255B%257B%22field%22:%22date_range%22,%22value%22:%255B%222026-07-16+00:00:00%22,%222026-07-16+23:59:59%22%255D,%22label%22:%222026-07-16+00:00:00~2026-07-16+23:59:59%22%257D,%257B%22field%22:%22assignee_id%22,%22value%22:%257B%22id%22:47,%22username%22:%22li2%22%257D,%22label%22:%22li2%22%257D,%257B%22field%22:%22labels%22,%22value%22:%255B%257B%22id%22:18516,%22name%22:%22documentation%22%257D%255D,%22label%22:%22documentation%22%257D,%257B%22field%22:%22order_by_sort%22,%22value%22:%22created_at_asc%22,%22label%22:%22%E8%BE%83%E6%97%A9%E5%88%9B%E5%BB%BA%22%257D%255D&state=opened&order_by=created_at&sort=asc&scope=all&page=1&dateRange=2026-07-16+00:00:00&dateRange=2026-07-16+23:59:59
}
```

```python
    if successful_repos:
        print(f"\n正在从 {DATA_FILE} 中移除 {len(successful_repos)} 个已成功处理的仓库...")
        try:
            # 读取所有仓库
            with open(DATA_FILE, 'r', encoding='utf-8') as f:
                all_repos = [line.strip() for line in f if line.strip()]
            
            # 移除成功的仓库
            remaining_repos = [repo for repo in all_repos if repo not in successful_repos]
            
            # 写回文件
            with open(DATA_FILE, 'w', encoding='utf-8') as f:
                for repo in remaining_repos:
                    f.write(repo + '\n')
            
            print(f"✓ 已从文件中移除 {len(successful_repos)} 个成功的仓库")
            print(f"  剩余 {len(remaining_repos)} 个仓库待处理")
        except Exception as e:
            print(f"✗ 移除成功仓库时出错: {e}")
```


https://test.gitcode.net/li2/recommend/issues/68?ref=&did=aa37e9dbf713b8d2b7c6c2b763e8cfc1b4d3849b#tid-2264608


https://test.gitcode.net/li2/recommend/issues?categorysearch=%255B%257B%22field%22:%22date_range%22,%22value%22:%255B%222026-07-16+00:00:00%22,%222026-07-16+23:59:59%22%255D,%22label%22:%222026-07-16+00:00:00~2026-07-16+23:59:59%22%257D,%257B%22field%22:%22assignee_id%22,%22value%22:%257B%22id%22:47,%22username%22:%22li2%22%257D,%22label%22:%22li2%22%257D,%257B%22field%22:%22labels%22,%22value%22:%255B%257B%22id%22:18516,%22name%22:%22documentation%22%257D%255D,%22label%22:%22documentation%22%257D,%257B%22field%22:%22order_by_sort%22,%22value%22:%22created_at_asc%22,%22label%22:%22%E8%BE%83%E6%97%A9%E5%88%9B%E5%BB%BA%22%257D%255D&state=opened&order_by=created_at&sort=asc&scope=all&page=1&dateRange=2026-07-16+00:00:00&dateRange=2026-07-16+23:59:59
